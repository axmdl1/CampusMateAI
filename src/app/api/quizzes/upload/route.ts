...
export async function POST(request: NextRequest) {
    try {
        ...
        // Optional: Clear previous store for this simple prototype session
        clearVectorStore();

        const sessionId = await processDocument(file);

        return NextResponse.json({ success: true, sessionId, message: 'Document processed successfully' });
    } catch (error) {
